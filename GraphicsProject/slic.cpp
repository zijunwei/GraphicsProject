...
void Slic::generate_superpixels(IplImage *image, int step, int nc) {
	...
	for (int i = 0; i < NR_ITERATIONS; i++) {
		...
		for (int j = 0; j < (int)centers.size(); j++) {
			...
			for (int k = (int) centers[j][3] - step; k < centers[j][3] + step; k++) {
				for (int l = (int)centers[j][4] - step; l < centers[j][4] + step; l++) {
...
void Slic::display_center_grid(IplImage *image, CvScalar colour) {
	for (int i = 0; i < (int)centers.size(); i++) {
		cvCircle(image, cvPoint((int)centers[i][3],(int)centers[i][4]), 2, colour, 2);
	}
}
...
void Slic::colour_with_cluster_means(IplImage *image) {
	//vector<CvScalar> colours(centers.size()+1);

	///* Gather the colour values per cluster. */
	//for (int i = 0; i < image->width; i++) {
	//	for (int j = 0; j < image->height; j++) {
	//		int index = abs( clusters[i][j]);
	...
	for (int i = 0; i < image->width; i++) {
		for (int j = 0; j < image->height; j++) {
			CvScalar ncolour =255*abs( clusters[i][j]);
